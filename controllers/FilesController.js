...
import baseUtils from '../utils/basic';
...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    if (!baseUtils.isValid(userId)) return response.status(401).send({ error: 'Unauthorized' });
    ...
    if (fileParams.parentId !== 0 && !baseUtils.isValidId(fileParams.parentId)) { return response.status(400).send({ error: 'Parent not found' }); }
